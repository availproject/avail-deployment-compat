...
echo "  - Maddr: /dns/{{ lb_dns }}/tcp/{{ hostvars[item].tags.AvailPort }}/p2p/{{ lookup('op', 'Wallet Credentials for ' + hostvars[item].tags.Name, vault='Avail Devnet: ' + clean_deploy_title, field='libP2PPub') | trim }}"
{% endif %}
{% endfor %}
